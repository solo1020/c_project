...
```
这个循环 getchar() 逐个字符读取当前行直到\n :  
while( (ch = getchar()) != '\n' ){    
    putchar(ch);   
}    
然后一个字符一个字符的输出, 但是因为缓冲区的存在，所以不是读取一个字符后立即输出，而是放在缓冲区,在遇到 \n 后一次性从缓冲区中取出进行输出    

## 函数
函数前置声明一般放在主函数main之前，也可以直接放在主调函数里面  
只要保证程序在第一次使用函数前知道函数的类型包括返回值类型和参数类型， 即先声明函数原型再使用    

这里类似于Object-C的头文件h中声明函数再在另一个源文件中定义函数

```
#include <stdio.h>
int main(void){
    int imin(int, int); /*声明imin函数的原型*/
...
上述读取输入时：  
如果正确输入This is the value头 + 整数 则读取成功  
如果错误输入THis is the value头 + 整数 读取失败  
如果不输入This is the value头，直接输入整数，读取成功  
其他细节：  
scanf("%*s") 会忽略所有字符串直到下一个空白符
printf("%*d",5,s)会用 5 去替代 * 即格式化输出  
...
#### gcc编译：
gcc -o 输出程序的名称(不需要带后缀名) file1.c file2.c  

scanf("%*s") 跳至下一个空白符  
#### 逻辑表达式求值规则：
while ( (status = scanf("%d", &code)) != 1 ||(code < 1 || code > 5))    
从左往右求值，一旦左边求值结果为假,不对右边进行求值  



##### 不使用新变量存储值 如何交换两个变量的值
int a;  
int b;  
// 计算a b 差值  
a = a - b;   

// 此时a已经赋值为 差值, b + 差值 即为a, 因为交换变量,所以将 b + 差值  赋给 b;   
b = b + a;

// 此时 b 已经是交换前a的值, 要获取原来b的值并赋给a, 用原来 a的值减去 差值(现在的a)即可   
a = b - a;   
...
只有在定义的时候 *p 代表的是 指针   
其余时候  *p 表示 用 间接运算符取出指针所在地址的值    
普通变量将值作为基本量，把地址作为通过&运算符获得的派生量   
而指针变量将地址作为基本量，把值作为通过 *间接运算符获得的派生量   

函数的返回类型和形参列表构成了函数签名---- 和Java很像 方法签名  

数组和指针  
const数组不能将值写入数组   
sizeof 计算对象的占用字节大小   
  
#### 数组首元素指针(地址) 也即 数组变量的指针(地址)  
指针的+ 是指 增加一个存储单元 具体一个单元为多少字节根据指针指向的数据类型决定  
int dates[SIZE]  
dates + 2 == &dates[2]    
*(dates + 2) == dates[2]    

函数形参定义只能在函数原型和定义头中使用int arr[] 代替int *arr   
